...
char *_dbfnames_to_mmap(FILE *fp_db)
{
  ...
  size_mmap = sizeof(int) + sizeof(s_file_name) * num_files; 
  ...
  *p_num_files = num_files;
  ...
  for(i = 0; i < num_files; i++) {
    ...
    strcpy(f_ori.key, line);
    sem_init(&f_ori.sem, 1, 1);

    f_dest = (s_file_name *) pointer;
    
    strcpy(f_dest->key, f_ori.key);
    f_dest->sem = f_ori.sem;
    f_dest->done = 0;

    pointer += sizeof(s_file_name); 
  }
...
char *get_dbfname_from_mmap(char *mmap, int i)
{
  ...
  if (i < 0) 
  {
    fprintf(stderr, "ERROR in get_fname. Value of i = %d\n", i);
    exit(1);
  }
...
char *_get_dbfname_from_mmap(char *mmap, int i)
{
  ...
  pointer = mmap + sizeof(int) + i * sizeof(s_file_name);
